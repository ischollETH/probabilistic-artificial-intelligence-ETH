...
class Model(object):
    ...
    def __init__(self):
        ...
        learning_rate = 1e-3
        hidden_layers = (64, 64)  # for each entry, creates a hidden layer with the corresponding number of units
        use_densenet = False
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    log_likelihood = F.nll_loss(F.log_softmax(output_features, dim=1), batch_y, reduction='sum')
                    kl = log_variational_posterior - log_prior
                    loss = kl / num_batches + log_likelihood
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.weights_var_posterior = MultivariateDiagonalGaussian(
            # torch.nn.Parameter(Normal(0.0, 0.05).sample((out_features, in_features))),
            torch.nn.Parameter(0.01 * torch.ones((out_features, in_features))),
            torch.nn.Parameter(-4 * torch.ones((out_features, in_features)))
        )
        ...
        if self.use_bias:
            ...
            self.bias_var_posterior = MultivariateDiagonalGaussian(
                # torch.nn.Parameter(torch.squeeze(Normal(0.0, 0.05).sample((out_features, 1)))),
                torch.nn.Parameter(0.01 * torch.ones(out_features)),
                torch.nn.Parameter(-4 * torch.ones(out_features))
            )
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        if self.use_bias:
            ...
            log_prior = self.prior.log_likelihood(weights).sum() + self.prior.log_likelihood(bias).sum()
            log_variational_posterior = self.weights_var_posterior.log_likelihood(weights).sum() + self.bias_var_posterior.log_likelihood(bias).sum()
        else:
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for idx, current_layer in enumerate(self.layers):
            new_features, new_log_prior, new_log_variational_posterior = current_layer(output_features)
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, rho: torch.Tensor):
        ...
        self.mu = mu
        self.rho = rho
        self.negHalfLog2PI = -.5 * math.log(2.0 * math.pi)
        np.random.seed(77)
        self.gaussian_block = np.random.randn(10000)

    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        return self.negHalfLog2PI - torch.log(F.softplus(self.rho)) - (values - self.mu) ** 2 / (2 * F.softplus(self.rho) ** 2)

    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        np.random.seed(42)
        epsilon = np.random.choice(self.gaussian_block, size=torch.numel(self.mu))
        epsilon = np.expand_dims(epsilon, axis=1).reshape(self.mu.shape)
        epsilon = torch.from_numpy(epsilon).type(torch.FloatTensor)
        epsilon = Variable(epsilon)
        return self.mu + F.softplus(self.rho) * epsilon
