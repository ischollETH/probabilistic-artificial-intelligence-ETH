...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        of chosen action.
        """
        # TODO1: Implement this function. --> DONE!!
        ...
        with torch.no_grad():
            policy, logp_a = self.pi.forward(state)
            action = policy.sample()
            policy, logp_a = self.pi.forward(state, action)
            value_fct = self.v.forward(state)
        return action.item(), value_fct, logp_a
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        # TODO6: Implement computation of phi. --> DONE!
        ...
        # ------------------------C)-------------------------------------------------------------------------------
        self.ret_buf[path_slice] = discount_cumsum(rews[:-1], self.gamma)*(self.gamma**np.arange(self.ptr-self.path_start_idx))
    ...
    def get(self):
        ...
        # TODO7: Here it may help to normalize the values in self.phi_buf --> DONE!
        phi_sigma = np.std(self.phi_buf)
        phi_mu = np.mean(self.phi_buf)
        self.phi_buf = (self.phi_buf-phi_mu)/phi_sigma
...
class Agent:
    def __init__(self, env):
        ...
        # Learning rates for policy and value function --> originally pi_lr = 3e-3, vf_lr = 1e-3
        pi_lr = 6e-3
        vf_lr = 7e-4
    ...
    def pi_update(self, data):
        ...
        # Options for computing the pi_loss
        # --------------A)----------------------------
        # pi_loss = -torch.mean(phi * logp_a)
        # --------------B)----------------------------
        pi_loss = -torch.sum(phi * logp_a)
        # --------------C)----------------------------
        # pi_loss = -torch.mean((phi * logp_a).sum())
        # --------------D) without baseline----------------------------
        # pi_loss = -torch.mean(ret*logp_a)
        # --------------E) with baseline-----------------------------
        # pi_loss = -torch.mean((ret-phi)*logp_a)
        # --------------F) with baseline-----
        # value_fct = self.ac.v.forward(obs)
        # pi_loss = -torch.mean(torch.mul(phi-value_fct, logp_a))
    ...
    def v_update(self, data):
        ...
        for _ in range(100):
            ...
            # normalization
            # v_sigma = torch.std(value_fct)
            # v_mu = torch.mean(value_fct)
            # value_fct = (value_fct-v_mu)/(v_sigma)

            # Options for the v_loss:
            # ----------A)---------------------------------------
            # v_loss = -torch.sum(phi*value_fct)
            # ----------B)---------------------------------------
            # smooth_l1_loss = nn.SmoothL1Loss(reduction='mean')
            # v_loss = smooth_l1_loss(value_fct, ret)
            # ----------C)---------------------------------------
            mean_squared_error = nn.MSELoss()
            v_loss = mean_squared_error(value_fct, ret)
            # ----------D)---------------------------------------
            # v_loss = -torch.sum((value_fct - phi).pow(2))
            # ----------E)---------------------------------------
            # gamma = 0.99
            # target = torch.tensor(ret + gamma * value_fct)
            # v_value_network = self.ac.v(obs)
            # mean_squared_error = nn.MSELoss()
            # v_loss = mean_squared_error(v_value_network, target)
    ...
    def train(self):
        ...
        # Number of training steps per epoch --> originally 3000
        steps_per_epoch = 3000
        # Number of epochs to train for --> originally 50
        epochs = 50
        # The longest an episode can go on before cutting it off --> originally 300
        max_ep_len = 300
        # Discount factor for weighting future rewards --> originally gamma = 0.99, lam = 0.97
        gamma = 0.99
        lam = 0.97
    ...
    def get_action(self, obs):
        ...
        """
        # TODO3: Implement this function. --> DONE!
