...
def log_posterior_probs(x):
    ...
    log_p = np.zeros(3)
    inner_sum = np.zeros(3)

    for j in range(3):
        inner_sum[j] = np.log(PRIOR_PROBS[j]) + np.sum(np.log(HYPOTHESIS_SPACE[j].pdf(x)))

    for i in range(3):
        log_p[i] = np.log(PRIOR_PROBS[i]) + np.sum(np.log(HYPOTHESIS_SPACE[i].pdf(x))) - logsumexp(inner_sum)
